<?php
define('STRIPE_API_KEY', 'sk_test_51HuWprGmKHCBqlqFfMNUqcjS7QHv5i1kgaSFKVlnHy8LLAya2TwG1KutK5avkZOSW16CW4U3TeUI7RC862XFhEDG00eLWtDMIY'); 
define('STRIPE_PUBLISHABLE_KEY', 'pk_test_51HuWprGmKHCBqlqF99N4QZgUEdE8Hw5drSw1pjNIXUcQdTHfVVjZ4fjThnGjYrNR99y2jKJEPei92gLQ8fFGuqf600wjiHlEsl'); 
...
if (!function_exists('autogenlp_theme_scripts_styles')) {

    function autogenlp_theme_scripts_styles() {
        // Theme info
        $my_theme = wp_get_theme();
        $path_url = network_site_url().'/wp-content/themes/autogenlp';
        // Fonts & enqueue css
        ...
        if(!empty($template_name) && (($blog_id != 1) || (is_page('preview')) || (is_page('demos'))) && ($template_name!='lp-template-surviral')){
            $template_name = str_replace('lp-template-', '', trim(strtolower($template_name)));
            wp_enqueue_style('aglp_style-css', $path_url . '/css/style.css');
            wp_enqueue_style($template_name . '-style', $path_url . '/css/'.$template_name.'-style.css');
            wp_enqueue_style('aglp_responsive-css', $path_url . '/css/responsive.css');
    ...
    add_action('wp_enqueue_scripts', 'autogenlp_theme_scripts_styles');
}
...
function lp_pages_style_files(){
    ...
    wp_enqueue_style('aglp-admin', get_template_directory_uri() . '/css/admin-style.css');
    wp_enqueue_style('aglp-fontawsome', 'https://use.fontawesome.com/releases/v5.0.1/css/all.css');
     // Localize the script with new data
...
function email_verification_call_back(){
    ...
    if(!isset($_SESSION['code_generator']) || empty($_SESSION['code_generator']) || $_POST['code_status'] == 'resend_code'  ){
        ...
        $subject    = "Email Verification for Auto Generate LP.";
        $body       = "Your verification code is - ".$code_generator;
        $headers    = "From: Auto Generate LP";
...
*/
add_action( 'network_admin_menu', 'aglp_network_admin_menus' );
function aglp_network_admin_menus(){
...
function algp_transaltion(){ 
        ...
        get_template_part( 'template-parts/admin-translation/aglp-translation' );
...
*/
function template_admin_tabs( $current = 'surviral' ) {
    // List of Tabs array.
    $tabs_list = array('surviral' => 'Surviral' , 'xamulai' => 'Xamulai');
    $tabs = apply_filters( 'aglp_tab_item_list', $tabs_list);
    echo '<h2 class="nav-tab-wrapper">';
    foreach( $tabs as $tab => $name ){
        $class = ( $tab == $current ) ? ' nav-tab-active' : '';
        echo "<a class='nav-tab$class' href='?page=aglp-lp-settings&tab=$tab'>$name</a>";
    }
    echo '</h2>';
}
 ...
 */
function aglp_network_lp_settings_cb(){
    ...
    <div class="wrap">
        ...
        ?>
        <div class="lp-panel custom-ui-aglp">
            <div class="lp-panel-tab-section"> 
                <form method="post" action="edit.php?action=aglp_lp_update">
                <input type="hidden" name="aglp_network_options" value="update" />
                    <?php
                    wp_nonce_field( "aglp_lp_settings_nonce" ); 
                        
                    echo '<table class="form-table">';
                    switch ( $tab ){
                        case 'surviral' :
                            $surviralFields = array(
                                array(
                                    'name' => 'section_1',
                                    'label' => 'Banner' ,
                                ),
                            ...
                            <p>By default all sections are visible on the landing page. You can uncheck the box right side of the section to hide that section</p>
                                <?php foreach($surviralFields as $skey => $sfield) { 
                                   $count = $skey + 1;
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input name="surviral[<?php echo $fieldName; ?>]" type="checkbox" <?php if ($isChecked) echo 'checked="checked"'; ?> value="true" />
                                    </td>
                                </tr>
                                <?php } ?>
                            <?php
                            ...
                            case 'xamulai' :
                                ...
                                <h2>Landing Page section show / hide options</h2>
                                <p>By default all sections are visible on the landing page. You can uncheck the box right side of the section to hide that section</p>
                                    <?php foreach($xamulaiFields as $skey => $sfield) { 
                                       $count = $skey + 1;
                                       $fieldName = $sfield['name'];
                                       $fieldLabel = $sfield['label'];
                                       $isChecked = $autolpSettings[$fieldName];
...
*/
add_action( 'network_admin_edit_aglp_lp_update', 'template_save_settings_options' );
function template_save_settings_options($network_id) {
    check_admin_referer('aglp_lp_settings_nonce');

    // Saving Surviral LP values
    if (isset($_POST['aglp_network_options'])) {
        $fieldValue = serialize($_POST['surviral']);
        update_network_option($network_id, 'aglp_network_options', $fieldValue );
    }
  
	wp_redirect( add_query_arg( array(
		'page' => 'aglp-lp-settings',
		'updated' => true ), network_admin_url('admin.php?page=aglp-lp-settings')
	));
...
function network_settings_update_notices(){
	if( isset($_GET['page']) && $_GET['page'] == 'aglp-lp-settings' && isset( $_GET['updated'] )  ) {
...
* Create new sub site
*/ 
//create_new_site($domain = '/', $path = 'autogenlp/akshaysite2', $title = "Sub site", $userid = '1');
...
* Save meta data
*/
add_action( 'wp_ajax_update_aglp_seo_meta', 'update_aglp_seo_meta_call_back');
add_action( 'wp_ajax_nopriv_update_aglp_seo_meta', 'update_aglp_seo_meta_call_back');
function update_aglp_seo_meta_call_back(){
    ...
    if(!empty($user_id)){
        ...
        // Save to user meta
        update_user_meta($user_id, 'aglp_meta_image', $meta_image);
        update_user_meta($user_id, 'aglp_meta_title', $meta_title);
        update_user_meta($user_id, 'aglp_meta_desc' , $meta_desc);
        update_user_meta($user_id, 'aglp_meta_gsv' , $meta_gsv);
        update_user_meta($user_id, 'aglp_meta_gas' , $meta_gas);
...
function get_all_template(){
    $template_array = array(
        'lp-template-surviral' => 'Surviral',
        'lp-template-xamulai' => 'Xamulai'
    );
...
function lp_template_select($blog_id){
    ...
    if($template_name == 'lp-template-xamulai'){
        ...
        return get_template_part('template-parts/lp-template-surviral');
    }
